...
public class ExportVocabularyDialog {
	...
	public static void export(Shell shell, VocabularyInProject vocabularyInProject) {
		Vocabulary vocabulary = OpenIIManager.getVocabulary( vocabularyInProject.getProjectID() );
		Project project = OpenIIManager.getProject(vocabulary.getProjectID());
		...
		if (filename != null) {
			try {
				...
				exporter.exportVocabulary(vocabulary, new File(filename)); 
