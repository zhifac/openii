...
public class ManagerAction extends Action {
	...
	ManagerAction(ManagerMenuManager menuManager, String title,
		...
		case KEYWORD_SEARCH:
			icon = "Search.gif";
			break;
		case CREATE_PROJECT_FROM_TAG:
			icon = "Project.gif";
			break;
		case EXPORT_SCHEMAS_BY_TAG:
			icon = "Export.gif";
			break;
		case DELETE_TAG_SCHEMA:
			icon = "Delete.gif";
			break;
		case NEW_PROJECT:
			icon = "Project.gif";
		...
		case IMPORT_MAPPING:
			...
			break;
		case REPLACE_SCHEMA:
			icon = "ReplaceSchema.gif";
			break;
		case AUTO_GENERATE_MATCHES:
			icon = "Mapping.gif";
			break;
		case EXPORT_MAPPING:
			icon = "Export.gif";
	...
	public void run() {
		...
		if (actionType == ActionType.EDIT_TAG)
			new EditTagDialog(shell, (Tag) selection, ((Tag) selection)
					.getParentId()).open();
		...
		if (actionType == ActionType.KEYWORD_SEARCH)
			new SearchDialog(shell, (selection instanceof Tag ? (Tag) selection
					: null)).open();
		...
		if (actionType == ActionType.DELETE_TAG_SCHEMA) {
			...
			ArrayList<Integer> schemaIDs = OpenIIManager
					.getTagSchemas(tagSchema.getTagID());
		...
		if (actionType == ActionType.DELETE_PROJECT_SCHEMA) {
			...
			Project project = OpenIIManager.getProject(schemaInProject
					.getProjectID());
			ArrayList<ProjectSchema> schemas = new ArrayList<ProjectSchema>(
					Arrays.asList(project.getSchemas()));
		...
		if (actionType == ActionType.DELETE_VOCABULARY)
			DeleteVocabularyDialog.delete(shell,
					(VocabularyInProject) selection);

		if (actionType == ActionType.EXPORT_VOCABULARY)
			ExportVocabularyDialog.export(shell,
					(VocabularyInProject) selection);
