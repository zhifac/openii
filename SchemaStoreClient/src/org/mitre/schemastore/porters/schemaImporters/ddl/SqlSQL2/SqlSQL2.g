...
rule_def:
    "create" "rule" (~(SEMICOLON))* SEMICOLON
;
...
//{ Rule #611 <unique_constraint_def>gh
unique_constraint_def :
	  unique_spec (LEFT_PAREN /*unique_column_list*/column_name_list RIGHT_PAREN)? ( "using" "index" LEFT_PAREN (~RIGHT_PAREN)* RIGHT_PAREN)?
;
//}
...
refd_table_and_columns :
	table_name { builder.addReferenceTo( returnAST.toString() ); } (LEFT_PAREN /*ref_column_list*/column_name_list RIGHT_PAREN)?
;
//}
...
//{ Rule #441 <ref_triggered_action> incorporatees the rule #620 <update_rule> and #160 <delete_rule>
...
//{ Rule #092 <column_def>
column_def :
	column_name { builder.addAttributeToLastEntity( returnAST.toString() ); } (data_type | domain_name) (default_clause)? (column_constraint_def)* (collate_clause)?
;
//}
...
alter_table_stmt :
	"alter" "table" table_name { System.out.println("Alter table statement for " +  returnAST.toString()); builder.setEntityTo( returnAST.toString()); } alter_table_action
;
//}
