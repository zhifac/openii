		...
		try {
			...
			for (MappingCell mappingCell : client.getMappingCells(mapping.getId())) {
				if (mappingCell.isValidated()) {
					...
					if (isSource) {
						referenceIDs.add(mappingCell.getOutput());
					} else {
						referenceIDs.addAll(Arrays.asList(mappingCell.getInput()));
					}
					...
					if (isSource) {
						associatedIDs.addAll(Arrays.asList(mappingCell.getInput()));
					} else {
						associatedIDs.add(mappingCell.getOutput());
					}
