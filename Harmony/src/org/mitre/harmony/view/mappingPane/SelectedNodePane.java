...
class SelectedNodePane extends JPanel implements SelectedInfoListener
{
	...
	public void selectedElementsModified(Integer side)
	{
		...
		// Determine if this pane should be made visible
		List<Integer> elements = SelectedInfo.getElements(side);
		if(elements.size()!=1) { setVisible(false); return; }
		SchemaElement element = SchemaManager.getSchemaElement(elements.get(0));
		setVisible(true);
