...
public class Permuter implements java.util.Enumeration<Pair> {
	...
	public Permuter(ArrayList<?> objList) throws Exception {
		...
		if (this.items.size() <= 1) throw new Exception("Cannot permute empty or singleton list of files!");
	}
	...
	public void exclude(String pattern) throws Exception {
		if (started) throw new Exception("Really bad idea to exclude files after you've already started.  Forget about it.");

		ArrayList<Object> filtered = new ArrayList<Object>();

		for (int x = 0; x < items.size(); x++) {
			if (items.get(x).toString().matches(pattern)) continue;
	...
	public boolean hasMoreElements() {
		if (tailIDX == (items.size() - 1) && headIDX == (tailIDX - 1)) return false;

		if (headIDX >= items.size() - 1) return false;
		return true;
	} // End hasMoreElements
