...
public class ClusterDistanceSliderPane extends JPanel {
	...
	private class ClusterDistanceSlider extends JSlider
	{
		...
		{	
			...
			public void paintClusters(Graphics g) {
				...
				if(orientation == JSlider.VERTICAL) {
					...
					if(computeClusterTickLocations) {
						...
						// Paint the clustering indicator tick mark locations on the slider
						for(Integer clustTickY : clusterTickLocations) {
							g.fillRect(8, clustTickY, 6, 1);
						}
					}
				}
					...
					if(computeClusterTickLocations) {	
						//System.out.println("drawing ticks, min=" + min + ", max=" + max);
						...
						// Paint the clustering indicator tick mark locations on the slider
						for(Integer clustTickX : clusterTickLocations) {
							g.fillRect(clustTickX, yCoord, tickWidth, tickHeight);
						}
					}
				}
			...
			public void moveMaxThumbUp(int numClicks) {
				...
				if(Math.abs(clusterTickLocations.get(clusterTickIndex)-currPos) < 2) {
					if(clusterTickIndex < clusterTickLocations.size() - 1) {						
						setMaxPos(clusterTickLocations.get(++clusterTickIndex));						
					}
				}
				else {
					for(clusterTickIndex = 0; clusterTickIndex < clusterTickLocations.size(); clusterTickIndex++) {					
						int clusterTickLocation = clusterTickLocations.get(clusterTickIndex);
				...
				if(origPos != maxValue) {
					...
					this.maxD = clusterTickLocationsD.get(clusterTickIndex);
					this.minD = minValue/1000D;
					this.fireClusterDistanceChangeEvent();
				}
			}
		...
		private ClusterDistanceSlider(ClustersContainer clusters, int orientation)
		{
			...
			minValue = 0;
			maxValue = 1000;
			//minValue = new Double(Filters.getMinConfThreshold()*scaleRange).intValue();
			//maxValue = new Double(Filters.getMaxConfThreshold()*scaleRange).intValue();
		}
	}
	...
	public ClusterDistanceSliderPane(ClustersContainer clusters, int orientation)
	{
		...
		//Add the slider
		final ClusterDistanceSlider slider = new ClusterDistanceSlider(clusters, orientation);
