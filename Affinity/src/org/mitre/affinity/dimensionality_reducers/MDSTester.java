...
public class MDSTester {
	...
	public MDSTester(ArrayList<Integer> schemas, final DistanceGrid dg, final PositionGrid pg) {
		...
		for(Integer sID : schemas) {
			...
			OvalSchemaGUI schemaGUI = new OvalSchemaGUI(shell, sID, schemaDiameter, schemaDiameter);			
		...
		final Schema2DView schema2DPlot = new Schema2DView(schema2DPlotPane, SWT.NONE, schemaGUIs);
		...
		final List<ISchemaGUI> widgetDistances = new ArrayList<ISchemaGUI>();
		...
		for(Integer s1=1; s1<=numSchemas; s1++) {
			...
			for(Integer s2=s1; s2<=numSchemas; s2++) {				
				if(s1 != s2) {					
					...
					OvalSchemaGUI schemaGUI = new OvalSchemaGUI(shell, s1, schemaDiameter, schemaDiameter);			
		...
		final Schema2DView shephardPlot = new Schema2DView(shephardPlotPane, SWT.NONE, widgetDistances);
	 ...
	 * @return
	 */
	public static double compute2DFitCorrelation(ArrayList<Integer> schemas, DistanceGrid dg, PositionGrid pg) {
		double sumX = 0;
		double sumY = 0;
		double sumXSquared = 0;
		double sumYSquared = 0;
