...
{
	/** If true, IM-PLUS will be activated and provenance will be captured on all actions.  If false, that code is disabled. */	 
	...
	static {
		if(CAPTURE_PROVENANCE) addListener(IMPLUSListener.primary);
		reset(); 
	} // End static
	...
	public static ArrayList<Integer> getAncestorSchemas(Integer schemaID)
		{ try { return RepositoryManager.getClient().getAncestorSchemas(schemaID); } catch(Exception e) { return new ArrayList<Integer>(); } }	
