...
public class Dependency {
	...
	public Object[] generateMapping ()
	{
		...
		MappingSchema[] mappingSchemas = new MappingSchema[2];
		...
		MappingSchema source = new MappingSchema(sourceSchema.getId(),sourceSchema.getName(),"RMap",MappingSchema.LEFT);
		MappingSchema target = new MappingSchema(targetSchema.getId(),targetSchema.getName(),"RMap",MappingSchema.RIGHT);
		mappingSchemas[0] = source;
		mappingSchemas[1] = target;
		Mapping mapping = new Mapping(LogicalRelation.getNextID(),"","","",mappingSchemas);
	...
	static public ArrayList<Dependency> generateDependencies(ArrayList<LogicalRelation> sourceLogRels, 
				ArrayList<LogicalRelation> targetLogRels, 
				ArrayList<MappingCell> correspondences){
		...
		for (LogicalRelation sourceLogRel : sourceLogRels ){
			for (LogicalRelation targetLogRel : targetLogRels){
				...
				for (MappingCell corr : correspondences){
					if (sourceLogRel.translate_SS_to_LR(corr.getInput()) != null  
						&& targetLogRel.getIDmappings_SS_to_LR().get(corr.getOutput()) != null ){
	...
	public static ArrayList<Dependency> generate(Integer mappingID, SchemaStoreClient client){
		...
		try {
			for (MappingSchema mapSchema : client.getMapping(mappingID).getSchemas()){
				if (mapSchema.getSide().equals(MappingSchema.LEFT)){
					SchemaInfo sourceGraph = client.getSchemaInfo(mapSchema.getId()).copy();
					renderRelational(sourceGraph);
					allSourceLogRels.addAll(LogicalRelation.createLogicalRelations(sourceGraph));
					
				} else if (mapSchema.getSide().equals(MappingSchema.RIGHT)){
					SchemaInfo targetGraph = client.getSchemaInfo(mapSchema.getId()).copy();
					renderRelational(targetGraph);
					allTargetLogRels.addAll(LogicalRelation.createLogicalRelations(targetGraph));
				
				} else
					System.err.println("[E] Generator:generate -- schema " + mapSchema.getName() + " must be assigned a side");
			}
