...
import backend.SQLGenerator;


@SuppressWarnings("serial")
public class SQLGeneratorGUI extends JPanel implements MappingListener {
	...
	public SQLGeneratorGUI(SchemaStoreClient client, Integer elementID) {
		super();
        ...
        _harmonyModel.getMappingManager().addListener(this);
        ...
        /** create checkbox */
        JRadioButton derbyButton = new JRadioButton(SQLGeneratorGUI.DERBY_TYPE,true);
        derbyButton.setName(SQLGeneratorGUI.DERBY_TYPE);
        derbyButton.setSelected(true);
        JRadioButton postgresButton = new JRadioButton(SQLGeneratorGUI.POSTGRES_TYPE,true);
        postgresButton.setName(SQLGeneratorGUI.POSTGRES_TYPE);
	...
	class SelectDBListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (button.getName().equals(SQLGeneratorGUI.DERBY_TYPE))
				_targetDB = SQLGeneratorGUI.DERBY_TYPE;
			else if (button.getName().equals(SQLGeneratorGUI.POSTGRES_TYPE))
				_targetDB = SQLGeneratorGUI.POSTGRES_TYPE;
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
	    	...
	    	// save current work (for visible Dependency)
	   		...
	   		if (_dependDisplayedIndex >= 0){
	   			ArrayList<MappingCell> mappingCells = _harmonyModel.getMappingCellManager().getMappingCells();
	   		...
	   		if (selectedIndices.size() > 0){ 
	   			...
	   			if (finalScript.size() == 0)
	   				finalScript = SQLGenerator.generateFinalSQLScript(selectedDepends, _targetDB);
			...
			if (colIndex == 1 )
		        ...
		        if (_harmonyModel.getSchemaManager()._mappingID_by_Dependency != null) {
		        	...
		        	if (mappingIDdisplayed != null){
		        		 ...
		        		 mappingSchemas.add(_harmonyModel.getSchemaManager()._sourceSchemaIDForMappingID.get(mappingIDdisplayed));
		        		 mappingSchemas.add(_harmonyModel.getSchemaManager()._targetSchemaIDForMappingID.get(mappingIDdisplayed));    		 
		        		 _harmonyModel.getMappingManager().loadMapping(mappingIDdisplayed);	 
		        		 _harmonyModel.getMappingManager().setSchemas(mappingSchemas);
		        		 _mappingPane.getTree(MappingSchema.LEFT).expandNode(_mappingPane.getTree(MappingSchema.LEFT).root);
		        	     _mappingPane.getTree(MappingSchema.RIGHT).expandNode(_mappingPane.getTree(MappingSchema.RIGHT).root);
		        	} 
		        } 	
			} 
		}
       ...
       javax.swing.SwingUtilities.invokeLater(new Runnable() {
           public void run() {
       		...
       		try { 
       	        ...
       	        JComponent newContentPane = new SQLGeneratorGUI(client, 422130);
