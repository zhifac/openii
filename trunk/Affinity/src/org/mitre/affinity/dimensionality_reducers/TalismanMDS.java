 ...
 *
 */
public class TalismanMDS implements IMultiDimScaler {
	private static TalismanMDS talismanMDSInstance;
	
	private TalismanMDS() {}
	
	public static TalismanMDS getTalismanMDSInstance() {
		if(TalismanMDS.talismanMDSInstance == null)
			TalismanMDS.talismanMDSInstance = new TalismanMDS();
		return TalismanMDS.talismanMDSInstance;
	}
	...
	public PositionGrid scaleDimensions(DistanceGrid dg, boolean isSymmetric, boolean isMetric, 
		...
		for(SchemaPairValue<Double> value : dg.getValues())
			table.addValue(value.getSchema1ID(), value.getSchema2ID(), value.getValue());
